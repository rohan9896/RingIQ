...
function FlowNode({ icon: Icon, title, note, highlighted = false, compact = false }) {
  return (
    <div
      className={`relative flex min-w-0 items-center gap-2 rounded-lg border px-2.5 py-2 backdrop-blur-md ${
        highlighted
    ...
    >
      ...
      <div className="min-w-0">
        <p className={`${compact ? 'text-[0.66rem]' : 'text-xs'} truncate font-display font-semibold text-text-primary`}>
          {title}
        </p>
        <p className="mt-0.5 truncate font-body text-[0.55rem] text-text-muted">{note}</p>
      </div>
    </div>
  );
}
...
export default function ArchitectureSlide() {
  return (
    <div className="slide-page relative bg-bg-base text-text-primary">
      ...
      >
        <motion.div variants={rise} className="mb-2 flex items-center gap-3">
        ...
        <motion.p variants={rise} className="mt-2 font-body text-base text-text-secondary">
      ...
      <div className="slide-content relative z-10 flex min-h-0 flex-col justify-center gap-2.5">
        <motion.div
          className="grid grid-cols-[7.5rem_1fr] overflow-hidden rounded-xl border border-border-default/70 bg-bg-card/55 backdrop-blur-xl"
          initial="hidden"
          animate="show"
          transition={{ staggerChildren: 0.06, delayChildren: 0.12 }}
        >
          <div className="flex flex-col justify-between border-r border-border-subtle bg-bg-base/45 px-3 py-3">
            <div>
          ...
          <div className="grid grid-cols-[1fr_2rem_1fr_2rem_1fr_2rem_1fr] items-center gap-1.5 p-2.5">
        ...
        <motion.div
          className="grid grid-cols-[7.5rem_1fr] overflow-hidden rounded-xl border border-primary-300/25 bg-primary-500/10 backdrop-blur-xl"
          initial="hidden"
          animate="show"
          transition={{ staggerChildren: 0.06, delayChildren: 0.28 }}
        >
          <div className="flex flex-col justify-between border-r border-primary-300/20 bg-bg-base/40 px-3 py-3">
            <div>
          ...
          <div className="relative p-2.5">
            <div className="grid grid-cols-[1fr_2rem_1fr_2rem_1fr_2rem_1fr] items-center gap-1.5">
            ...
            <div className="mt-1.5 flex items-center justify-between px-1 font-body text-[0.55rem] text-text-muted">
              <span>Production: each call already loads pinned tenant KB context</span>
        ...
        >
          ...
          <div className="p-2.5">
            <div className="grid grid-cols-[1fr_1.25rem_1fr_1.25rem_1fr_1.25rem_1.12fr_1.25rem_1fr_1.25rem_0.78fr] items-center gap-1">
            ...
            <div className="mt-1.5 flex items-center justify-between px-1 font-body text-[0.55rem] text-text-muted">
              <span className="flex items-center gap-1.5"><Fingerprint className="h-3 w-3 text-primary-300" /> tenant_7f · campaign_82 · lead_204 · attempt_9</span>
              <span className="flex items-center gap-1.5"><CheckCircle2 className="h-3 w-3 text-primary-300" /> Idempotent events + result → core, asynchronous to media</span>
            </div>
          </div>
        </motion.div>

        <motion.section
          className="grid grid-cols-4 overflow-hidden rounded-xl border border-primary-300/25 bg-primary-500/10 backdrop-blur-xl"
        ...
        >
          {invariants.map(({ icon: Icon, title, text }, index) => (
            <div key={title} className={`px-3 py-2.5 ${index > 0 ? 'border-l border-border-subtle' : ''}`}>
              ...
              <p className="mt-1 font-body text-[0.54rem] leading-[1.35] text-text-muted">{text}</p>
            </div>
          ))}
        </motion.section>
      </div>
